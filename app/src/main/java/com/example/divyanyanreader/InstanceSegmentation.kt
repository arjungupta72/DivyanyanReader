...
import com.example.divyanyanreader.ImageUtils.clone
import com.example.divyanyanreader.ImageUtils.scaleMask
import com.example.divyanyanreader.MetaData.extractNamesFromLabelFile
import com.example.divyanyanreader.MetaData.extractNamesFromMetadata
...
class InstanceSegmentation(
    ...
    init {
        ...
        // STEP 1: Try NNAPI First (Faster initialization: 6-10s)
        try {
            ...
            Log.i("YOLO", " Initialized using NNAPI (Fast Init: ~6-10s)")
            message("Using NNAPI acceleration")
        } catch (e: Exception) {
            Log.e("YOLO", " NNAPI Initialization Failed: ${e.message}")
            nnApiDelegate?.close()
            nnApiDelegate = null
        }

        // STEP 2: Try GPU with FAST_SINGLE_ANSWER if NNAPI failed
        if (interpreterInstance == null) {
            try {
                ...
                Log.i("YOLO", " Initialized using GPU (Balanced Init: ~12-15s)")
